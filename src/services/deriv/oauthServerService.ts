...
function attemptFetchProfileWithUrl(token: string, wsUrl: string): Promise<DerivAccountProfileData | null> {
  return new Promise((resolve) => {
    try {
      ...
      const sendAuth = () => {
        try {
          if (ws.readyState === 1 || ws.readyState === WS.OPEN) {
...
export async function handleDerivOAuthCallback(params: {
  ...
  if (token1 && acct1) {
    const cleanToken1 = token1.trim();
    if (!cleanToken1) {
      return {
        success: false,
        destination: '/?auth_error=token_failed&message=Empty%20token%20provided%20in%20callback',
        errorMessage: 'Deriv OAuth Error: Provided token1 query parameter is empty.',
      };
    }
  ...
  try {
    ...
    const cleanResolvedToken = resolvedAccessToken ? String(resolvedAccessToken).trim() : '';
    if (!cleanResolvedToken) {
      return {
        success: false,
    ...
    const hydrationResult = await hydrateDerivAccount({
      ...
      accessToken: cleanResolvedToken,
      appId: oauthConfig.clientId,
